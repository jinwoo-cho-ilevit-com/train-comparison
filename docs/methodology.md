...
**gemma-4-E2B는 2026-08-03 캠페인에서 제외됐다**(`PLAN.md` "gemma-4-E2B 제외" —
A100 80GB에 full FT가 batch 16/4 어느 쪽에서도 들어가지 않는다, 실측). 아래의
분석과 "조치 선택지" 결정은 이 모델을 대상으로 한 것이므로 이제 실행 대상이
아니다. 기록은 남긴다 — 같은 구조(오디오 마커가 vision 마커 목록에 없는 것)의
모델이 다시 후보에 오르면 그때 참조할 근거이기 때문이다.
...
## 12. 이미지 픽셀 캡 — 왜 캡을 두는가 (결정: 2026-08-03)

**결정: `max_pixels=1310720`, `min_pixels=4096`, 두 모델(Qwen3-VL-Embedding-2B,
Qwen3.5-0.8B)에 동일 적용. `data.max_seq_len`은 2048로 그대로 둔다.** 이 절이
근거를 적는 이유는 이것이 하네스 취향이 아니라 **측정 유효성 문제**이기 때문이다
— `docs/CONTRACTS.md`의 공유 파일 규율과 같은 이유로, 캡을 어디에 걸지가 아니라
왜 거는지는 여기가 정본이다.

**이 결정은 이 문서를 쓴 시점 코드에 반영되지 않았다.** `trainbench/config_schema.py`와
`configs/model/*.yaml`을 확인했고 `max_pixels`/`min_pixels` 필드가 없다 — 다른
레인이 구현 중이며, 착지 여부는 이 문서가 답하지 않는다.

### 왜 필요한가 — 두 모델의 픽셀 범위가 12.8배 벌어진다

visual token 하나는 `patch_size² x merge_size²` = 1024 픽셀을 덮는다(두 모델 모두
`patch_size=16`, `merge_size=2`). 두 체크포인트가 선언하는 픽셀 범위는 크게 다르다:

| 모델 | 선언된 `longest_edge` | 이미지 1장의 최대 visual token |
|---|---|---|
| Qwen3-VL-Embedding-2B | 1,310,720 | 1,280 |
| Qwen3.5-0.8B | 16,777,216 | 16,384 |

12.8배 차이다. 캡을 두지 않으면 메모리와 시간이 **그 런이 우연히 뽑은 이미지가
무엇이었는가의 함수**가 된다 — 이것은 `docs/CONTRACTS.md` §2의 baseline 3%-편차
게이트가 전제하는 "같은 축은 같은 조건"을 정면으로 깬다: 같은 config를 두 번
돌려도 샘플링된 이미지의 해상도 분포가 다르면 편차가 축 자체보다 커질 수 있다.

**코퍼스 실측: 배치 하나가 overflow할 수 있다.** 고정 revision
(`jinwoo-cho/mmeb-subset`, `b750b9c3263e9ef5dce225fd50aa25d7c58f1d5f`)에서 스캔한
결과 이미지 토큰이 최대 16,281까지 올라가는 행이 있다 — Qwen3.5의 상한 근처다.
텍스트 쪽은 코퍼스 전체에서 `max=270` 토큰이다. 캡을 걸었을 때의 최악 행은
`270 + 1280 = 1550`으로 `data.max_seq_len=2048` 안에 들어온다. **캡 없이는** 같은
계산이 `270 + 16281`로 2048을 가볍게 넘는다 — 배치 크기 1짜리 런도 이 값을 만나면
그 자리에서 죽거나(모델별 실패 모드에 따라) 시퀀스가 잘린다.

### 무엇을 바꾸지 않는가

`data.max_seq_len=2048`은 그대로다 — 캡을 건 뒤의 최악 행(1550)이 이미 그 안에
들어오므로 늘리거나 줄일 이유가 없다. 캡은 두 모델에 **동일한 값**으로 적용된다 —
모델마다 다른 캡을 주면 이것도 결정 3이 포기한 "모델 간 토큰 예산 고정"을 다른
이름으로 되살리는 것이 된다. 이 캡은 그 반대다: 각 모델이 자기 픽셀 범위 안에서
움직이되, 측정을 오염시킬 만큼 큰 이상치만 잘라낸다.
