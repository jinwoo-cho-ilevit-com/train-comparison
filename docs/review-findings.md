...
## D13 — `axis-values`가 "축 기계가 받는가"와 "이 연구가 켤 수 있는가"를 한 숫자로 뭉갰다 (2026-08-02, loss 레인 제보)

아홉 번째. 그리고 **감사 레인 자신의 직전 보고가 틀렸던 사례**이기도 하다.

`loss=cached_mnrl`은 구현이 서 있고 `axes._split_rows`가 `pixel_values`를 실은 배치를
전부 거부한다. 이 연구의 두 서브셋은 모두 이미지를 싣는 MMEB 드로우이므로, 이 값은
**텍스트 전용 서브셋에는 적용되고 이 연구가 설정한 어떤 런에도 적용되지 않는다.**
그런데 감사는 텍스트 전용 fixture 하나만 넘겨서 `loss 2/2`로 셌다.

**정정**: D11에서 감사 레인은 `loss` 축의 이동을 "진짜 개선(구현된 축이 미구현으로
오보되고 있었다)"이라고 보고했고, baseline note가 그 문장 위에 작성됐다. 절반만
맞았다 — `dataset=None` 때문에 거부되던 것은 감사의 결함이 맞지만, 이미지 데이터에서의
거부는 사실이다. `axes._gradcache_needs_splittable_data`의 docstring이 이미 이 점을
`axis-values`를 이름으로 지목해 적어두고 있었는데, 그것을 읽지 않고 숫자만 보고
"개선"으로 판정한 것이 오류의 경로다.

**수리 방식**: fixture를 이미지 쪽으로 교체하지 않았다. 그러면 같은 fixture를 쓰는
dataloader 축의 의미가 함께 바뀐다(제보가 지적한 그대로). 대신 **행이 이미지를 싣는지만
다른 두 fixture**로 각 값을 시도하고, 한쪽에서만 통과하는 값은 세지 않고 이름을 부른다.
"이 값은 데이터에 의존한다"가 선언이 아니라 **관측**이 되므로, MMEB가 이미지를 싣는다는
지식을 체크에 하드코딩하지 않아도 된다. 실측으로 두 fixture가 갈라놓는 값은
`loss/cached_mnrl` 하나뿐임을 확인했다.
