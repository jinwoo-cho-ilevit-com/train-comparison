...
**스택도 confound 다.** 이미지마다 transformers 와 torch 버전이 다르고 이것은 반복
측정으로 평균 낼 수 없다. 실무적으로도 "프레임워크를 고른다"는 것이 곧 "그 스택을
고른다"는 뜻이므로, 각 셀 옆에 버전을 표시한다 (Lane V 머지 완료). **정확한 값은
`envs/*/uv.lock` 에서 이미지별로 읽는다** — 이 파일에 매핑을 적어두면 그것이 곧
드리프트한다. 확인된 것 하나: `envs/ms-swift/uv.lock:2127` 이 transformers 5.12.1.

**커널도 confound 이고, 이것이 셋 중 유일하게 두 모델 반쪽을 가른다.**
`axes.py:127` 의 `FLA_ARCHS = {qwen3_5}` 를 `:384` 가 강제해 다른 아키텍처의
`kernel=fla` 를 거부한다. canonical 은 `kernel=fla` 를 핀하는데 그 이유가 타당하다 —
transformers 가 Qwen3.5 모델링 모듈을 import 하면서 fla 를 바인딩하므로 **그
아키텍처에는 fla 없는 런이 존재하지 않고**, `kernel=none` 은 fla 의 클래스로 만든
모델에 붙인 라벨이 된다. 결과: `qwen3_5` 여섯 셀은 fla 로 돌고 `qwen3_vl` 여섯은
그럴 수 없다. 두 모델 행을 가로질러 읽으면 "프레임워크 X 는 VL 에서 느리다" 가
나오는데 그것은 이 표가 답한 것이 아니다.

**셋 다 측정 오차가 아니라 구조다.** 오차로 쓰면 더 재면 줄어든다고 읽힌다.
...
`_baselines.yaml` 이 경고하는 그 드리프트가 생긴다 — 13줄 × 12파일이다.

확정된 형태: 매니페스트에 `inherit: canonical` 을 두고, canonical 의 overrides 에서
`model=`/`framework=` 를 뺀 나머지를 앞에 깔고 매니페스트 자신의 `overrides` 를
뒤에 붙인다(뒤가 이긴다). `model`/`framework` 는 이미 매니페스트의 최상위 필수
키이고(`orchestrate.py:62`), `overrides` 는 이미 선택 키로 있으며
`pod_overrides(exp)` 가 모든 런 앞에 붙인다 — 새 개념이 아니라 기존 둘을 잇는 것이다.

이 형태를 고르는 이유는 중복 제거만이 아니다. **`qwen3_vl` 여섯 매니페스트가
`kernel=none` 한 줄만 갖게 되고, 그 한 줄이 곧 위 커널 confound 의 문서가 된다.**
복사해 두면 그 편차가 13줄 속에 묻힌다.
